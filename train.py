...
from bert_model import BertBasedSentimentModel,BertBasedLSTMGRU,BertBasedSentimentModel2,BertBasedSentimentModel_last3
...
def train_epoch(model, optimizer,criterion, args):
...
def train(args):
    ...
    model = BertBasedSentimentModel_last3(HIDDEN_DIM, DROPOUT_PROB ,args).to(args.device)
    ...
    for epoch in range(n_epoch):
        train_loss = train_epoch(model,optimizer,criterion,args)
